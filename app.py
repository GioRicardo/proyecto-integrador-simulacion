...
def inicio_de_simulacion():
    ...
    for i in range(1, 21):
        cliente = {
            ...
            "bonificacion": 0,
            "service_time": 0,
            "tiempo_espera": "Pendiente",
            "tiempo_salida": "Pendiente"
        }
        valor_aleatorio1 = generate_random_value()
        tell = calculate_expression(valor_aleatorio1)
        current_time += tell
...
def mover_a_buffer():
    ...
    # Calcular el tiempo de salida de las cajas
    tiempos_salida = [caja["tiempo_salida"] if caja is not None else float('inf') for caja in cajas]
    menor_tiempo_salida = min(tiempos_salida)
    ...
    # Organizar la cola por tiempo de llegada (hill) en orden ascendente
    cola.sort(key=lambda x: x["hill"])

    while cola and len(buffer) < 3:
        cliente = cola.pop(0)
        
        if cliente["tipo_cliente"] == "Especial" and contar_tipo_cliente("Especial") > 0:
            continue
        ...
        elif cliente["tipo_cliente"] == "Transferido" and contar_tipo_cliente("Transferido") > 0:
            continue
...
def mover_a_cajas():
    ...
    for i in range(len(cajas)):
        if cajas[i] is None and buffer:
            cliente = buffer.pop(0)
            cliente["tiempo_espera"] = max(0, current_time - cliente["hill"])
            cliente["tiempo_salida"] = current_time + cliente["service_time"]
            cajas[i] = cliente
...
def simulacion():
    ...
    while cola_espera or buffer or any(cajas):
        ...
        current_time += 1  # Avanza el tiempo en cada iteración

        for i in range(len(cajas)):
            if cajas[i] is not None:
                cajas[i]["service_time"] -= 1
                datos_cajas.append(cajas[i].copy())
                if cajas[i]["service_time"] <= 0:
                    cajas[i] = None
